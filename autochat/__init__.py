...
class Autochat:
    ...
    def prepare_messages(
        ...
        if self.context:
            ...
            if isinstance(first_message.content, str):
                ...
                first_message.content = [
                    MessagePart(type="text", text=self.context),
                    *first_message.content,
                ]
    ...
    def run_conversation(
        ...
        if question:
            message = Message(
                role="user",
                content=question,  # This will become a text part internally
            )
            yield message
        else:
            message = None
    ...
    def fetch_anthropic(self):
        def add_empty_function_result(messages):
            ...
            for i in range(len(messages) - 1, 0, -1):
                # 1) Si c’est un message function juste avant un user
                if messages[i - 1].role == "function" and messages[i].role == "user":
                    func_msg = messages.pop(i - 1)

                    # On va chercher un éventuel MessagePart contenant l'ID et le contenu
                    # (typiquement 'function_call' ou 'function_result').
                    function_part = next(
                        (
                            p
                            for p in func_msg.parts
                            if p.type in ["function_call", "function_result"]
                        ),
                        None,
                    )

                    # On crée un MessagePart "tool_result", que la méthode to_anthropic_dict() saura convertir
                    tool_result_part = MessagePart(
                        type="function_result",  # À l'export, ce deviendra {"type": "tool_result", ...}
                        content=(
                            function_part.content
                            if (function_part and function_part.content)
                            else ""
                        ),
                        function_call_id=(
                            function_part.function_call_id if function_part else None
                        ),
                    )

                    # On insère ce part au début du message utilisateur suivant
                    user_msg = messages[i - 1]
                    user_msg.parts.insert(0, tool_result_part)

                # 2) Sinon, si c’est un assistant avec un function_call, suivi d’autre chose que 'function'
                elif (
                    messages[i - 1].role == "assistant"
                    and getattr(messages[i - 1], "function_call", None)
                    and messages[i].role != "function"
                ):
                    # On insère un message function vide (logique existante pour Anthropic)
                    # On récupère éventuellement l'ID depuis messages[i - 1].function_call["id"] si vous l’y stockez
                    messages.insert(
                        i,
                        Message(
                            role="function",
                            ...
                            content="",
                            function_call_id=messages[i - 1].function_call.get("id"),
                        ),
                    )
